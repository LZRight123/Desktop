...
struct hook_t hook_list[] = {// system 没替换
    //反反调试
    {ptrace, "/usr/lib/system/libsystem_kernel.dylib", "ptrace"}, //1
    {dlsym, "/usr/lib/system/libdyld.dylib", "dlsym"}, //2
    {sysctl, "/usr/lib/system/libsystem_c.dylib", "sysctl"}, //3
    {syscall, "/usr/lib/system/libsystem_kernel.dylib", "syscall"}, //4
    {isatty, "/usr/lib/system/libsystem_c.dylib", "isatty"}, //5
    {ioctl, "/usr/lib/system/libsystem_kernel.dylib", "ioctl"}, //6
    {task_get_exception_ports, "/usr/lib/system/libsystem_kernel.dylib", "task_get_exception_ports"}, //7
    //反反越狱文件权限
    {strstr, "/usr/lib/system/libsystem_platform.dylib", "strstr"}, //8
    {dladdr, "/usr/lib/system/libdyld.dylib", "dladdr"}, //9
    {fopen, "/usr/lib/system/libsystem_c.dylib", "fopen"}, //10
    {fopen, "/usr/lib/system/libsystem_c.dylib", "fopen"}, //11
    {access, "/usr/lib/system/libsystem_kernel.dylib", "access"}, //12
    {stat, "/usr/lib/system/libsystem_kernel.dylib", "stat"}, //13
    {lstat, "/usr/lib/system/libsystem_kernel.dylib", "lstat"}, //14
    {getenv, "/usr/lib/system/libsystem_c.dylib", "getenv"}, //15
    {_dyld_image_count, "/usr/lib/system/libdyld.dylib", "_dyld_image_count"}, //16
    {_dyld_get_image_name, "/usr/lib/system/libdyld.dylib", "_dyld_get_image_name"}, //17
    //{system}
    {creat, "/usr/lib/system/libsystem_c.dylib", "creat"}, //18
    {openat, "/usr/lib/system/libsystem_kernel.dylib", "openat"}, //19
    {fork, "/usr/lib/system/libsystem_c.dylib", "fork"}, //20
    {popen, "/usr/lib/system/libsystem_c.dylib", "popen"}, //21
    //有一个注释位
};
void refreshHook_list(){
    ...
    //反反调试
    hook_list[1].func_ptr = ptrace;
    hook_list[2].func_ptr = dlsym;
    hook_list[3].func_ptr = sysctl;
    hook_list[4].func_ptr = syscall;
    hook_list[5].func_ptr = isatty;
    hook_list[6].func_ptr = ioctl;
    hook_list[7].func_ptr = task_get_exception_ports;
    //反反越狱文件权限
    hook_list[8].func_ptr = strstr;
    hook_list[9].func_ptr = dladdr;
    hook_list[10].func_ptr = fopen;
    hook_list[11].func_ptr = fopen;
    hook_list[12].func_ptr = access;
    hook_list[13].func_ptr = stat;
    hook_list[14].func_ptr = lstat;
    hook_list[15].func_ptr = getenv;
    hook_list[16].func_ptr = _dyld_image_count;
    hook_list[17].func_ptr = _dyld_get_image_name;
    //system 没有替换
    hook_list[18].func_ptr = creat;
    hook_list[19].func_ptr = openat;
    hook_list[20].func_ptr = fork;
    hook_list[21].func_ptr = popen;
}
...
int my_ptrace(int _request, pid_t _pid, caddr_t _addr, int _data){
    ...
    LZLog(@"[AntiAntiDebug] - ptrace request is PT_DENY_ATTACH");
    
    return 0;
}

void* my_dlsym(void* __handle, const char* __symbol){
    LZLog(@"通过dlsym调用 - dlsym get symbol %s",__symbol);
...
int my_sysctl(int * name, u_int namelen, void * info, size_t * infosize, void * newinfo, size_t newinfosize){
    if(namelen == 4 && name[0] == CTL_KERN && name[1] == KERN_PROC && name[2] == KERN_PROC_PID && info && infosize && ((int)*infosize == sizeof(_kinfo_proc))){
        ...
        if(info_ptr && (info_ptr->kp_proc.p_flag & P_TRACED) != 0){
            LZLog(@"[AntiAntiDebug] - sysctl query trace status.");
            info_ptr->kp_proc.p_flag ^= P_TRACED;
            if((info_ptr->kp_proc.p_flag & P_TRACED) == 0){
                LZLog(@"trace status reomve success!");
            }
        }
        return ret;
    }
...
#endif
        if(request == 31){
            LZLog(@"[AntiAntiDebug] - syscall call ptrace, and request is PT_DENY_ATTACH");
            return 0;
        }
    }
...
static int my_isatty(int i){
    int r = orig_isatty(i);
    if (i == 1 && r) {
        LZLog(@"[AntiAntiDebug] - isatty symbol");
        return 0;
    }
    return r;
}
...
static int my_ioctl(int arg1, unsigned long arg2, ...){
    if (arg1 == 1 && arg2 == TIOCGWINSZ) {
        LZLog(@"[AntiAntiDebug] - ioctl symbol");
        return 1;
    }
    va_list args;
    va_start(args, arg2);
...
static FILE *my_fopen(const char * __restrict path, const char * __restrict mode){
    ...
    if (isJailbreakUTF8Path(path)) {
        LZLog(@"C函数 越狱检测1.1 fopen ===> %s", path);
        return NULL;
    }
...
static int my_access(const char *path, int mode){
    if (isJailbreakUTF8Path(path)) {
        LZLog(@"C函数 越狱检测1.3 access ===> %s", path);
        return -1;
    }
...
static int my_lstat(const char * path, struct stat * info){
    if (isJailbreakUTF8Path(path)) {
        LZLog(@"C函数 越狱检测2.1 lstat ===> %s", path);
        return -1;
    }
    ...
    if (!dyldArray) {
        ...
        while (count < orig_count) {
            ...
            if (image_name && !isJailbreakUTF8Path(image_name)) {
                LZLog(@"C函数 越狱检测4.1 orig__dyld_image_count ===> %s", image_name);
