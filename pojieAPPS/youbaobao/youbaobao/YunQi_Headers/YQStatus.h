//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class KJMarqueeLabel, UIImageView, UILabel, UIView, UIWindow;

@interface YQStatus : NSObject
{
    CDUnknownBlockType _tapBlock;
    UIWindow *_statusWindow;
    UIView *_backgroundView;
    UIImageView *_iconImageView;
    KJMarqueeLabel *_contentLabel;
    UILabel *_msgNewlabel;
}

+ (void)SY_UserIDChanged:(id)arg1;
+ (void)dismiss;
+ (void)showMessage:(id)arg1 whenTap:(CDUnknownBlockType)arg2;
+ (void)showMessage:(id)arg1 hideAfterDelay:(double)arg2 whenTap:(CDUnknownBlockType)arg3;
+ (void)showMessage:(id)arg1 hideAfterDelay:(double)arg2;
+ (void)showMessage:(id)arg1;
+ (id)sharedYQStatus;
@property(retain, nonatomic) UILabel *msgNewlabel; // @synthesize msgNewlabel=_msgNewlabel;
@property(retain, nonatomic) KJMarqueeLabel *contentLabel; // @synthesize contentLabel=_contentLabel;
@property(retain, nonatomic) UIImageView *iconImageView; // @synthesize iconImageView=_iconImageView;
@property(retain, nonatomic) UIView *backgroundView; // @synthesize backgroundView=_backgroundView;
@property(retain, nonatomic) UIWindow *statusWindow; // @synthesize statusWindow=_statusWindow;
@property(copy, nonatomic) CDUnknownBlockType tapBlock; // @synthesize tapBlock=_tapBlock;
- (void).cxx_destruct;
- (void)hide;
- (void)showMessage:(id)arg1 hideAfterDelay:(double)arg2 whenTap:(CDUnknownBlockType)arg3;
- (void)showMessage:(id)arg1 hideAfterDelay:(double)arg2;
- (void)tap:(id)arg1;
- (void)initContentLabel;
- (void)setup;

@end

