...
@class NSData, NSMutableArray, NSString, NSURL, YvGCDAsyncReadPacket, YvGCDAsyncSocketPreBuffer, YvGCDAsyncWritePacket;
@protocol OS_dispatch_queue, OS_dispatch_source, YvGCDAsyncSocketDelegate;

@interface YvGCDAsyncSocket : NSObject
{
    unsigned int flags;
    unsigned short config;
    id <YvGCDAsyncSocketDelegate> delegate;
...
+ (id)getIPAddresses;
+ (_Bool)isIpv6Net;
+ (id)getProperIPWithAddress:(id)arg1 port:(unsigned int)arg2;
+ (id)ZeroData;
+ (id)LFData;
+ (id)CRData;
+ (id)CRLFData;
...
@property __weak id <YvGCDAsyncSocketDelegate> delegate;
