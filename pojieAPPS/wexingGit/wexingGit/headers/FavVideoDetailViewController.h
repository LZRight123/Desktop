//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "FavMMUIViewController.h"

#import "FavForwardLogicDelegate-Protocol.h"
#import "IFavCdnDownloadMgrExt-Protocol.h"
#import "IFavoritesExt-Protocol.h"
#import "INetworkStatusMgrExt-Protocol.h"
#import "MMWebViewDelegate-Protocol.h"
#import "UIAlertViewDelegate-Protocol.h"
#import "WCActionSheetDelegate-Protocol.h"

@class FavDataItemWrap, FavoritesItem, FavoritesItemDataField, MMProgressView, MMWebViewController, NSString, UIButton, UIDocumentInteractionController, UIImageView, UILabel, UIView;
@protocol FavVideoDetailDelegate;

@interface FavVideoDetailViewController : FavMMUIViewController <MMWebViewDelegate, IFavCdnDownloadMgrExt, UIAlertViewDelegate, INetworkStatusMgrExt, IFavoritesExt, FavForwardLogicDelegate, WCActionSheetDelegate>
{
    FavoritesItem *m_favItem;
    FavDataItemWrap *m_favDataWrap;
    FavoritesItemDataField *m_favData;
    UIImageView *m_imageView;
    MMProgressView *m_progressBar;
    UILabel *m_labProgress;
    UIButton *m_btnVideoOpen;
    UIButton *m_btnOpenByApp;
    UIView *m_syncView;
    UILabel *m_labelSyncTip;
    UIButton *m_labelSync;
    UIDocumentInteractionController *m_vcDocument;
    int syncStautus;
    _Bool bFromRecord;
    _Bool m_bIsUnkownType;
    _Bool m_bIsFileExit;
    NSString *m_nsVideoPath;
    NSString *m_nsSteamUrl;
    unsigned int m_uiVideoSize;
    MMWebViewController *m_webViewController;
    UIView *m_loadingView;
    id <FavVideoDetailDelegate> m_delegate;
}

@property(nonatomic) __weak id <FavVideoDetailDelegate> m_delegate; // @synthesize m_delegate;
- (void).cxx_destruct;
- (id)sessionId;
- (id)getFavForawrdViewController;
- (void)updateRightBtn;
- (void)onFavTagEditBegin:(int)arg1;
- (void)actionSheet:(id)arg1 clickedButtonAtIndex:(long long)arg2;
- (void)onAction:(id)arg1;
- (void)onWebViewDidRotateFromInterfaceOrientation:(long long)arg1;
- (void)onNetworkStatusChange:(unsigned int)arg1;
- (void)OnUploadFavoritesItemFinish:(id)arg1 ErrCode:(int)arg2;
- (void)OnUploadFavoritesItem:(id)arg1 LocalDataId:(id)arg2 FinishedLength:(int)arg3 TotalLength:(int)arg4;
- (void)OnFavCdnDownload:(id)arg1 RetCode:(int)arg2;
- (void)OnFavCdnDownload:(id)arg1 TotalLength:(int)arg2 FinishLength:(int)arg3;
- (void)SetProgress:(double)arg1 TotalSize:(double)arg2;
- (void)StartSync;
- (void)OpenByApp;
- (void)OpenVideo;
- (void)showVideoExpire;
- (void)showSyncComplete;
- (void)showDownloading;
- (void)showUploading;
- (void)showNeedDownload;
- (void)showNeedUpload;
- (void)updateBtns;
- (void)updateStatus;
- (void)initView;
- (void)updateData;
- (void)viewDidLoad;
- (void)SafeDeleteDocument;
- (void)dealloc;
- (id)initWithFavDataWrap:(id)arg1;
- (id)initWithFavItem:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

