...
#pragma mark -
#pragma mark - 常量
static NSString * bunlderID = @"com.axe.ylyfbeeplay";
static NSString * myDylibName = @"BeePlayDylib";
...
static BOOL openAntiAntiDebug = true;
...
__attribute__((always_inline)) static BOOL AntiDebug_003()  {
    //需要转换的字符串
        NSString *dateString = @"2019-08-14";
//    设置转换格式
...
//    NSString转NSDate
        ...
        NSTimeInterval i = [date timeIntervalSinceDate:now];
    
    if (i < 0) {
        if (random() %2 == 0) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(7 * 60 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                asm_exit();
            });
        }
    }
...
__attribute__((constructor)) static void entryAntiDebug03(){
    AntiDebug_003();
}
